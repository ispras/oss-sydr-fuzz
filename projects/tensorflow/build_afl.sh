...
export LIB_FUZZING_ENGINE="/afl.o"
afl-clang-fast -g -fsanitize=undefined,address -c /afl.cc -o $LIB_FUZZING_ENGINE
...
sed -i -e 's/linkstatic/linkopts = \["\/afl.o -fsanitize=address,undefined -ldl"\],\nlinkstatic/' tensorflow/security/fuzzing/tf_fuzzing.bzl
...
for bazel_target in ${FUZZERS}; do
  ...
  cp ${bazel_location} /afl/$fuzz_name
done

rm -f bazel-*
rm -rf /root/.cache/bazel/_bazel_root

export LDFLAGS="-g -fsanitize=address,undefined -fno-sanitize=vptr"
export CFLAGS="-g -fsanitize=address,undefined -fno-sanitize=vptr"
export CXXFLAGS="-g -fsanitize=address,undefined -fno-sanitize=vptr"
