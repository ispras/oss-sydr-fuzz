...
./nss/build.sh $nss_flag --ubsan --asan --disable-tests --static -v -Dmozilla_client=1 -Dzlib_libs=$PREFIX/lib/libz.a -Dsign_libs=0
...
cmake .. -DBUILD_SHARED_LIBS=OFF -DBUILD_CODEC=OFF -DCMAKE_INSTALL_PREFIX=$PREFIX
...
if [ "$SANITIZER" != "memory" ]; then
    ...
    pushd $SRC/libpng
    autoreconf -fi
    CPPFLAGS="-fsanitize=address,integer,bounds,null,undefined,float-divide-by-zero -I$PREFIX/include" LDFLAGS=-L$PREFIX/lib ./configure --prefix="$PREFIX" --disable-shared --disable-dependency-tracking
    make -j$(nproc)
    make install
...
sed -i -e "s/QMAKE_CXXFLAGS    += -stdlib=libc++/QMAKE_CXXFLAGS    +=  $CXXFLAGS\nQMAKE_CFLAGS += $CFLAGS/g" mkspecs/linux-clang-libc++/qmake.conf
sed -i -e "s/QMAKE_LFLAGS      += -stdlib=libc++/QMAKE_LFLAGS      +=  -lpthread $CXXFLAGS/g" mkspecs/linux-clang-libc++/qmake.conf
