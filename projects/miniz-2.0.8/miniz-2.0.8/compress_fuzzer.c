...
static void check_mz_compress_level(uint8_t *compr, size_t comprLen,
                                 uint8_t *uncompr, size_t uncomprLen,
                                 int level)
{
    mz_compress2(compr, &comprLen, data, dataLen, level);
    mz_uncompress(uncompr, &uncomprLen, compr, comprLen);
...
static void write_zlib_header(uint8_t *s)
{
    ...
    unsigned int header = (MZ_DEFLATED + ((w_bits-8)<<4)) << 8;
...
static void check_decompress(uint8_t *compr, size_t comprLen)
{
    ...
    mz_uncompress(compr, &comprLen, copy, copyLen);
    free(copy);
}
...
int LLVMFuzzerTestOneInput(const uint8_t *d, size_t size)
{
    ...
    size_t comprLen = 100 + 2 * mz_compressBound(size);
    ...
    check_mz_compress_level(compr, comprLen, uncompr, uncomprLen, 1);
    check_mz_compress_level(compr, comprLen, uncompr, uncomprLen, 3);
    check_mz_compress_level(compr, comprLen, uncompr, uncomprLen, 6);
    check_mz_compress_level(compr, comprLen, uncompr, uncomprLen, 7);
