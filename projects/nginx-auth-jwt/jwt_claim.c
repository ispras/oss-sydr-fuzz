...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    if (jwt_decode(&jwt, str, NULL, 0)) {
        goto fuzzer_end;
    }
    ngx_http_auth_jwt_get_header(jwt, "fuzz1", "fuzz2", "fuzz3");
    ngx_http_auth_jwt_get_header_int(jwt, "fuzz1", "fuzz2", "fuzz3");
    ngx_http_auth_jwt_get_header_bool(jwt, "fuzz1", "fuzz2", "fuzz3");
    free(ngx_http_auth_jwt_get_headers_json(jwt, "fuzz1", "fuzz2", "fuzz3"));

    ngx_http_auth_jwt_get_grant(jwt, "fuzz1", "fuzz2", "fuzz3");
    ngx_http_auth_jwt_get_grant_int(jwt, "fuzz1", "fuzz2", "fuzz3");
    ngx_http_auth_jwt_get_grant_bool(jwt, "fuzz1", "fuzz2", "fuzz3");
    free(ngx_http_auth_jwt_get_grants_json(jwt, "fuzz1", "fuzz2", "fuzz3"));
