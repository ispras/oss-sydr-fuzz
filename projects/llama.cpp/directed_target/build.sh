...
build_lib() {
    cmake -B build$LIBAFL_SHARED_NAME -DLLAMA_CURL=OFF  -DCMAKE_C_COMPILER=$CC -DCMAKE_CXX_COMPILER=$CXX -DBUILD_SHARED_LIBS=OFF -DLLAMA_BUILD_SERVER=OFF
    cmake --build build$LIBAFL_SHARED_NAME --config Debug -j16
    ...
    mkdir -p ./build$LIBAFL_SHARED_NAME/myos
    find "./build$LIBAFL_SHARED_NAME/ggml/"   -name '*.o' -exec cp {} "./build$LIBAFL_SHARED_NAME/myos/" \;
    find "./build$LIBAFL_SHARED_NAME/src/"    -name '*.o' -exec cp {} "./build$LIBAFL_SHARED_NAME/myos/" \;
    find "./build$LIBAFL_SHARED_NAME/common/" -name '*.o' -exec cp {} "./build$LIBAFL_SHARED_NAME/myos/" \;
}

# For non-libafl target we can build libs only once.
if [[ "$TARGET" != "libafl" ]]
then
    export LIBAFL_SHARED_NAME=""
    build_lib
fi
...
build_target(){
    ...
    $CXX $CXXFLAGS ${FLAGS} ./$target.o \
        ...
        ./build$LIBAFL_SHARED_NAME/ggml/src/libggml.a ./build$LIBAFL_SHARED_NAME/common/libcommon.a \
