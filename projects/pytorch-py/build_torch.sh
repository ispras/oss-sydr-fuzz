...
CC=$CC CXX=$CXX CFLAGS=$CFLAGS CXXFLAGS=$CXXFLAGS MAX_JOBS=$(( $(nproc))) USE_ITT=0 USE_FBGEMM=0 BUILD_BINARY=0 USE_STATIC_MKL=1 \
    USE_DISTRIBUTED=1 USE_MPI=0 TP_BUILD_LIBUV=1 USE_TENSORPIPE=1 BUILD_CAFFE2_OPS=0 BUILD_CAFFE2=0 BUILD_TEST=0 \
    BUILD_SHARED_LIBS=ON USE_OPENMP=0 USE_MKLDNN=0 \
